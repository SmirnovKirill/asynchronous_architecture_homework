...
public class TaskController {
  ...
  @GetMapping("all")
  public List<Task> getAllTasks(@CookieValue(value = "userId", required = false) String userId) {
    if (StringUtils.isBlank(userId) || !authService.canViewAllTasks(userId)) {
  ...
  public List<Task> getCurrentUserTasks(@CookieValue(value = "userId", required = false) String userId) {
    if (StringUtils.isBlank(userId) || !authService.canViewOwnTasks(userId)) {
      throw new AuthException("Incorrect role for viewing current user tasks");
    }

    return taskService.getUserTasks(authService.getCurrentUser(userId));
  }

  @PostMapping("resolve/{taskId}")
  public void resolveTask(@CookieValue(value = "userId", required = false) String userId, @PathVariable long taskId) {
    if (StringUtils.isBlank(userId)  || !authService.canViewOwnTasks(userId)) {
    ...
    if (task.assignee().id() != authService.getCurrentUser(userId).id()) {
  ...
  @PostMapping("shuffle")
  public void shuffle(@CookieValue(value = "userId", required = false) String userId) {
    if (StringUtils.isBlank(userId)  || !authService.canShuffleTasks(userId)) {
