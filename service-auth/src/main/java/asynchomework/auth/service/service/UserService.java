...
public class UserService {
  ...
  public User save(CreateUser user) {
    PopugUserDb popugUserDb = userDao.save(
        new PopugUserDb(UUID.randomUUID().toString(), user.name(), user.beakSize(), user.role(), OffsetDateTime.now())
    );
    User savedUser = User.from(popugUserDb);

    kafkaProducer.sendUserCreated(savedUser);
    kafkaProducer.sendUserCreatedStream(savedUser);
